...
class StyleTransfer:
    ...
    def _build_graph(self):
        ...
        for id in self.Fs:
            if id in self.CONTENT_LAYERS:
                ...
                F = self.Fs[id] # content feature of x (init_image)
                P = self.Ps[id] # content feature of p (content_image)

                _, h, w, d = F.get_shape() # first return value is batch size (must be one)
                M = h.value * w.value # 각각의 필터의 가로와 세로를 곱한 값
                N = d.value # 필터의 갯수
                ## ==> Content Loss를 계산 하기 위해 feature map을 정의
                ...
                if self.content_loss_norm_type == 1:
                    ...
                    L_content += w * tf.reduce_sum(tf.pow((F-P), 2)) / (N*M) # artistic stlye transfer for videos
                ...
                M = h.value * w.value
                N = d.value
                ## ==> Style Loss를 계산 하기 위해 feature map을 정의

                w = self.STYLE_LAYERS[id] # weight for this layer

                G = self._gram_matrix(F) # style feature of x
                A = self.As[id] # style feature of a (style_image)

                L_style += w * (1. / (4 * N ** 2 * M ** 2)) * tf.reduce_sum(tf.pow((G-A), 2))

        # # total variation denoising
        # tv_weight = 1e2
        # tv_y_size = _tensor_size(self.init_image[:,1:,:,:])
        # tv_x_size = _tensor_size(self.init_image[:,:,1:,:])
        # tv_loss = tv_weight * 2 * (
        #         (tf.nn.l2_loss(self.init_image[:,1:,:,:] - self.init_image[:,:shape[1]-1,:,:]) /
        #             tv_y_size) +
        #         (tf.nn.l2_loss(self.init_image[:,:,1:,:] - self.init_image[:,:,:shape[2]-1,:]) /
        #             tv_x_size))

        # fix beta as 1 ==> for similar with style image
    ...
    def update(self):
        ...
        # this call back funcion is called every after loss is updated
        global _iter
        _iter = 0
    ...
    def _gram_matrix(self, tensor):
        ...
        # whick is assumed to be from a convolutional layer with 4-dim.
